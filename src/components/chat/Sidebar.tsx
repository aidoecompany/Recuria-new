...
export function Sidebar({ isOpen, onClose, onNewChat, sessions, onLoadSession }: SidebarProps) {
  return (
    <>
      {/* Mobile overlay */}
      <AnimatePresence>
...
>
        ...
        <div className="flex-1 overflow-y-auto pt-2 pb-4 scrollbar-thin">
          ...
          {sessions.length === 0 && (
            <p className="px-5 text-[12px] text-gray-300 italic">No sessions yet.</p>
          )}
          {sessions.map((session) => (
            <button
              ...
              className={`
                ...
                ${session.active ? "bg-gray-100 text-gray-950 font-medium" : "text-gray-500 hover:bg-gray-50"}
              `}
              style={{ width: "calc(100% - 16px)" }}
            >
              <span
                className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                style={{ background: session.active ? "#5a8a3a" : "#c7d8b0" }}
              />
